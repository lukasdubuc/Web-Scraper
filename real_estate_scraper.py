...
FIELDNAMES = ["title", "price", "beds_baths", "location",
              "date_posted", "url", "scraped_at", "city", "category"]
# ────────────────────────────────────────────────────────────────────────────
...
def get_listings_page(start: int) -> list[dict]:
    ...
    print(f"    Found {len(items)} raw items on page")
...
def save_to_csv(listings: list[dict], filename: str):
    """Save listings to a CSV file. Always creates file even if empty."""
